...
public class MethodData
{
    ...
    public MethodData baseMethodData; // 对应原始dll中的数据，因为新的method无法在原始dll中找到定义。
...
public class AssemblyDataBuilder
{
    ...
    public bool DoBuild(int patchNo)
    {
        ...
        assemblyData.allNewMethods.Clear();
        ...
        foreach(var method in assemblyData.allNewMethods.Values)
        {
            ...
            definition.HasThis = false;

            var pList = definition.parameters;

            var classType = definition.DeclaringType;

            var refType = MethodPatcher.Switch(assemblyData, classType);

            var pDefinition = new ParameterDefinition(refType);
            pDefinition.Name = "@this";

            pList.Insert(0, pDefinition);
    ...
    public bool FindNewAddMethods()
    {
        foreach (var kvT in assemblyData.newTypes)
        {
            ...
            TypeData newType = kvT.Value;
            // 暂不支持新增类型
            if (!assemblyData.baseTypes.TryGetValue(typeName, out TypeData baseType))
            {
                Debug.LogError($"can not find type `{typeName}` in old assembly[{assemblyData.name}]");
                return false;
            }
            ...
            var baseMethods = baseType.methods;
            var newMethods = newType.methods;
            foreach (var kvM in newMethods)
            {
                string methodName = kvM.Key;
                MethodData newMethod = kvM.Value;
                if (IsLambdaMethod(newMethod.definition)) // <MethodName>b_xx_y
                    continue;
    ...
    void FillMethodInfoField()
    {
        ...
        foreach (var kv in assemblyData.methodsNeedHook)
        {
            MethodData methodData = kv.Value.newMethod;
            Type t;
    ...
    void FixNewAssembly()
    {
        ...
        {
            var fixedType = new HashSet<TypeReference>();
            foreach(var kv in processed)
            {
                var status = kv.Value;
                if (status.ilFixed || status.needHook)
                {
                    fixedType.Add((kv.Key as MethodReference).DeclaringType);
                }
            }
            ...
            if(fixedType.Count > 0)
            {
                ...
                foreach (var tdef in fixedType)
                {
                    ...
                    foreach(var mdef in tdef.Resolve().Methods)
                    {
