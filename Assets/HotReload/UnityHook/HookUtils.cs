...
namespace MonoHook
{
    public static unsafe class HookUtils
    {
        ...
        private delegate void DelegateFlushICache(void* code, int size); // delegate * unmanaged[Cdecl] <void, byte, uint> native_flush_cache_fun_ptr; // unsupported at C# 8.0
        ...
        public static void SetAddrFlagsToRWX(IntPtr ptr, int size)
        {
            ...
            uint oldProtect;
            var ret = VirtualProtect(ptr, (uint) size, Protection.PAGE_EXECUTE_READWRITE, out oldProtect);
            Debug.Assert(ret);
#else
        ...
        public static KeyValuePair<long, long> GetPageAlignedAddr(long code, int size)
        {
            ...
            var startPage = code & ~(pagesize - 1);
            var endPage = (code + size + pagesize - 1) & ~(pagesize - 1);
        ...
        {
            Func<IntPtr, string> formatAddr = addr__ => IntPtr.Size == 4 ? $"0x{(uint) addr__:x}" : $"0x{(ulong) addr__:x}";
            ...
            while (true)
            {
                ...
                for (var i = 1; i < PRINT_COL_SIZE + 1; i++)
                {
                    if (count >= size)
                        goto END;
        ...
        private static readonly byte[] s_flush_icache_arm32 =
        {
        ...
        private static readonly byte[] s_flush_icache_arm64 =
        {
            // void cdecl mono_arch_flush_icache (guint8 *code, gint size)
