...
namespace MonoHook
{
    public abstract unsafe class CodePatcher
    {
        ...
        protected void BackupHeader()
        {
            ...
            var requireSize = LDasm.SizeofMinNumByte(_pTarget, _jmpCodeSize);
        ...
        {
            var buff = GenJmpCode(_pTarget, _pReplace);
        ...
        protected void PatchProxyMethod()
        {
            ...
            var jmpFrom = (long) _pProxy + _targetHeaderBackup.Length;
            var jmpTo = (long) _pTarget + _targetHeaderBackup.Length;

            var buff = GenJmpCode((void*) jmpFrom, (void*) jmpTo);
    ...
    {
        protected static readonly byte[] s_jmpCode =
        {
        ...
        protected override byte[] GenJmpCode(void* jmpFrom, void* jmpTo)
        {
            ...
            var val = (int) jmpTo - (int) jmpFrom - 5;

            fixed (void* p = &ret[0])
            {
                var ptr = (byte*) p;
                *ptr = 0xE9;
                var pOffset = (int*) (ptr + 1);
                *pOffset = val;
            }
    ...
    {
        private static readonly byte[] s_jmpCode =
        {
    ...
    public unsafe class CodePatcher_arm64_near : CodePatcher
    {
        ...
        {
            var ret = new byte[s_jmpCode.Length];
            var val = (int) ((long) jmpTo - (long) jmpFrom) / 4;
